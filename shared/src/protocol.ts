...
export const clientMessageSchema = z.discriminatedUnion('type', [
  ...
  z.object({
    ...
    payload: z.object({ roomId: z.string().min(1).max(64) }),
  }),
  z.object({ ...base, type: z.literal('room.leave'), payload: z.object({}) }),
  z.object({ ...base, type: z.literal('room.restart'), payload: z.object({}) }),
  z.object({ ...base, type: z.literal('room.lobby'), payload: z.object({}) }),
  z.object({
    ...base,
  ...
  z.object({
    ...
    payload: z.object({ matchId: z.string().min(1).max(64) }),
  }),
  z.object({
    ...base,
    type: z.literal('input.set'),
    payload: z.object({ sequence: z.number().int().nonnegative(), direction }),
  }),
])
...
export type GameModifierId = 'vortex' | 'pulse' | 'orbit' | 'wormhole'
