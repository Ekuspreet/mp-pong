...
beforeEach(() => {
  ...
  matches = createServices(config, db).matches
})
afterEach(() => {
  matches.close()
  db.close()
...
function setup() {
  ...
  return { app: createApp(config, db, services), rooms: services.rooms }
}
describe('authentication API', () => {
  it('creates a guest session that can immediately use protected APIs', async () => {
    ...
    expect((await agent.get('/api/auth/me').expect(200)).body.user.guest).toBe(
      true,
    )
  ...
  it('creates a guest with a requested retro call sign', async () => {
    ...
    const response = await agent
      .post('/api/auth/guest')
      .send({ username: 'Star-Marshal-X2V2' })
      .expect(201)
    expect(response.body.user).toMatchObject({
      username: 'Star-Marshal-X2V2',
      guest: true,
    })
    expect(
      (await agent.get('/api/auth/me').expect(200)).body.user.username,
    ).toBe('Star-Marshal-X2V2')
  })
  it('validates and reserves requested guest call signs', async () => {
    ...
    await request(app)
      .post('/api/auth/guest')
      .send({ username: 'Nova-Ranger-A1B2' })
      .expect(201)
    await request(app)
      .post('/api/auth/guest')
      .send({ username: 'nova-ranger-a1b2' })
  ...
  it('registers, restores, logs out, and protects APIs', async () => {
    ...
    await agent
      ...
      .send({ username: 'Player_1', password: 'password123' })
      .expect(201)
    expect(
      (await agent.get('/api/auth/me').expect(200)).body.user.username,
    ).toBe('Player_1')
  ...
  it('rejects duplicate usernames and bad credentials', async () => {
    ...
    await request(app)
      .post('/api/auth/register')
      .send({ username: 'Player_2', password: 'password123' })
      .expect(201)
    await request(app)
      .post('/api/auth/register')
      .send({ username: 'player_2', password: 'password123' })
    ...
    await request(app)
      .post('/api/auth/login')
      .send({ username: 'Player_2', password: 'wrongpass' })
      .expect(401)
  })
})
describe('rooms', () => {
  it('stores validated format and modifier identifiers on a room', async () => {
    ...
    const response = await agent
      .post('/api/rooms')
      .send({
        options: { format: 'stocks', modifiers: ['vortex', 'wormhole'] },
      })
      .expect(201)
    expect(response.body.room.options).toEqual({
      format: 'stocks',
      modifiers: ['vortex', 'wormhole'],
    })
    await agent
      .post('/api/rooms')
      .send({ options: { format: 'unknown', modifiers: [] } })
      .expect(400)
    await agent
      .post('/api/rooms')
      .send({ options: { format: 'elimination', modifiers: ['laser'] } })
      .expect(400)
  })
  it('enforces membership, readiness, and host transfer', () => {
    const rooms = createServices(config, db).rooms,
      a = { id: 'a', username: 'A', guest: false },
      b = { id: 'b', username: 'B', guest: false }
    const room = rooms.create(a)
    rooms.join(room.id, b)
    rooms.ready(a.id, true)
    rooms.ready(b.id, true)
    expect(rooms.canStart(room, a.id)).toBe(true)
    expect(rooms.canStart(room, b.id)).toBe(false)
    rooms.leave(a.id)
