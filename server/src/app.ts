...
export function createApp(config: Config, db: Db, services: Services) {
  ...
  if (config.NODE_ENV === 'production' && existsSync(clientDist)) { app.use(express.static(clientDist)); app.get('/{*splat}', (_req, res) => res.sendFile(fileURLToPath(new URL('index.html', `file://${clientDist}/`)))) }
